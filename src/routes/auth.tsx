...
import { LogIn, ArrowLeft, Mail, Lock, Sparkles, AlertCircle } from "lucide-react";
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      if (error) {
        ...
        toast.error("Google sign-in not configured. Please use email/password or contact support.");
        return;
      }
      // Supabase redirects the browser to Google itself, so there's nothing to do here.
    } catch (err) {
      console.error("[auth] Google OAuth exception:", err);
      toast.error("Google sign-in failed. Try using email/password instead.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="relative min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <div className="mx-auto max-w-md px-5 py-8">
        ...
        <button
          ...
          disabled={loading}
          className="mt-6 w-full rounded-2xl bg-surface-elevated border border-border py-3 text-sm font-semibold hover:border-teal/50 transition flex items-center justify-center gap-2 disabled:opacity-60"
        >
          <GoogleIcon />
...
function GoogleIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" aria-hidden>
      ...
      <path
        fill="#FBBC05"
        d="M5.84 14.12A6.99 6.99 0 015.5 12c0-.74.13-1.45.34-2.12V7.04H2.18A11 11 0 001 12c0 1.77.43 3.45 1.18 4.96l3.66-2.84z"
      />
      <path
        fill="#EA4335"
